export default function Cours01(props) {
  ...
  if (item) {
    ...
    const cour_pain = setInterval(() => {
      const cour_name = document.querySelector(".cours01 .cour_name span");

      if (cour_name) {
        if (cour_name.innerHTML === item.courName) {
          const cour_name_wrap = document.querySelector(".cours01 .cour_name");
    ...
    return (
      <div className="cours01">
        ...
        <div className="holes holes01">{createPar}</div>
      </div>
    );
  }
}
